    ...
    <Container>
      ...
      {(fetching && (
        ...
        <Row>
          ...
          {courses.map((course) => (
            <Col className="my-3" xs={6} md={4} lg={3} key={course.id}>
              <Card>
                <Card.ImageCap
                  variant="top"
                  src={buildLmsUrl(course.media.course_image.uri)}
                  srcAlt={intl.formatMessage(messages.catalogCourseBannerAlt, { courseName: course.name })}
                />
                <Card.Header title={course.name} />
                <Card.Footer>
                  <Button variant="primary" href={buildCourseURL(course.id)} block>{intl.formatMessage(messages.catalogCourseView)}</Button>
                </Card.Footer>
              </Card>
            </Col>
          ))}
